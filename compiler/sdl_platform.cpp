...
namespace dialos
{
    namespace vm
    {
        ...
        int SDLPlatform::timer_setInterval(const Value &callback, int ms)
        {
            ...
            entry.nextFire = std::chrono::steady_clock::now() + std::chrono::milliseconds(entry.intervalMs);
                ...
                if (now >= entry.nextFire)
                {
                    ...
                    if (vm_ != nullptr && vm_->isRunning() && !vm_->hasError())
                    {
                        ...
                        if (vm_->hasError() && !vm_->isRunning()) {
                            console_warn("VM entered error state during timer callback; clearing timers");
                    ...
                    // because the VM isn't running, avoid spamming the log.
                    if (attempted && !success)
                    {
                        console_warn("Timer callback id=" + std::to_string(entry.id) + " failed to invoke");
                    }
        ...
        void SDLPlatform::updateInputs()
        {
            ...
            if (duration.count() > 60 && !power_.charging)
            { // Update every minute
        ...
        int SDLPlatform::memory_allocate(int size)
        {
            ...
            console_warn("NOT IMPLEMENTED: os.memory.allocate - memory allocation not simulated");
            return -1; // Not implemented
        }
        ...
        void SDLPlatform::memory_free(int handle)
        {
            ...
            console_warn("NOT IMPLEMENTED: os.memory.free - memory deallocation not simulated");
        }
        ...
        std::vector<std::string> SDLPlatform::storage_getMounted()
        {
            ...
            console_warn("NOT IMPLEMENTED: os.storage.getMounted - storage enumeration partial/simulated");
        ...
        std::string SDLPlatform::storage_getInfo(const std::string &device)
        {
            ...
            console_warn("NOT IMPLEMENTED: os.storage.getInfo - storage info partial/simulated");
            if (device == "internal")
            {
        ...
        void SDLPlatform::printRuntimeError(const VMState &vm, const std::string &bytecodePath, size_t pc, const std::string &errorMessage, uint32_t sourceLine)
        {
            ...
            if (!sourceFile.empty()) {
                if (sourceLine > 0) {
                    ...
                    console_log("Debug info available but no line mapping for PC " + std::to_string(pc));
                }
            } else {
        ...
        std::string SDLPlatform::locateSourceForBytecode(const std::string &bytecodePath)
        {
            ...
            try {
                ...
                // 1) If the bytecode path has an accompanying .ds in the same folder, try it
                ...
                // 2) Try replacing extension in-place (handles cases where bytecode includes ext)
                fs::path replaced = p;
                replaced.replace_extension(".ds");
                if (fs::exists(replaced) && fs::is_regular_file(replaced))
                    return replaced.string();

                // 3) Try scripts/ directory next to repository (common location for .ds sources)
                fs::path repoScripts = fs::current_path() / "scripts" / (p.stem().string() + ".ds");
        ...
        std::string SDLPlatform::getSourceContextFromFile(const std::string &sourceFile, uint32_t errorLine, int contextLines)
        {
            ...
            size_t idx = (errorLine == 0) ? 0 : (std::min<uint32_t>(errorLine, (uint32_t)total) - 1);
                                          ...
                                          const ConsoleLog &log)
        {
                 ...
                 i++)
            {
                ...
                if (line.length() > 35)
                { // Rough character limit for console width
