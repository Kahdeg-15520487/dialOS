...
namespace dialos
{
    namespace compiler
    {
        ...
        BytecodeModule BytecodeCompiler::compile(const Program &program)
        {
            ...
            // Separate functions/classes from main code and track function names
            for (const auto &stmt : program.statements)
            {
                if (auto *funcDecl = dynamic_cast<const FunctionDeclaration *>(stmt.get()))
                {
                    functions.push_back(funcDecl);
                    declaredFunctions_.insert(funcDecl->name);
                }
                else if (auto *classDecl = dynamic_cast<const ClassDeclaration *>(stmt.get()))
                {
                    classes.push_back(classDecl);
                    // Track constructor and methods
                    if (classDecl->constructor) {
                        declaredFunctions_.insert(classDecl->name + "::constructor");
                    }
                    for (const auto& method : classDecl->methods) {
                        declaredFunctions_.insert(classDecl->name + "::" + method->name);
                    }
                }
                else
                {
            ...
            // If there were compilation errors, the bytecode may be incomplete
            // But we still return it for debugging purposes
            // The caller should check hasErrors() before using the module
        ...
        void BytecodeCompiler::compileBinaryExpression(const BinaryExpression &expr)
        {
            ...
            case BinaryExpression::Operator::ADD:
                emit(Instruction(Opcode::ADD), &expr);
                break;
            case BinaryExpression::Operator::SUB:
                emit(Instruction(Opcode::SUB), &expr);
                break;
            case BinaryExpression::Operator::MUL:
                emit(Instruction(Opcode::MUL), &expr);
                break;
            case BinaryExpression::Operator::DIV:
                emit(Instruction(Opcode::DIV), &expr);
                break;
            case BinaryExpression::Operator::MOD:
                emit(Instruction(Opcode::MOD), &expr);
                break;
            case BinaryExpression::Operator::EQ:
                emit(Instruction(Opcode::EQ), &expr);
                break;
            case BinaryExpression::Operator::NE:
                emit(Instruction(Opcode::NE), &expr);
                break;
            case BinaryExpression::Operator::LT:
                emit(Instruction(Opcode::LT), &expr);
                break;
            case BinaryExpression::Operator::LE:
                emit(Instruction(Opcode::LE), &expr);
                break;
            case BinaryExpression::Operator::GT:
                emit(Instruction(Opcode::GT), &expr);
                break;
            case BinaryExpression::Operator::GE:
                emit(Instruction(Opcode::GE), &expr);
                break;
            case BinaryExpression::Operator::AND:
                emit(Instruction(Opcode::AND), &expr);
                break;
            case BinaryExpression::Operator::OR:
                emit(Instruction(Opcode::OR), &expr);
                break;
            }
        }
